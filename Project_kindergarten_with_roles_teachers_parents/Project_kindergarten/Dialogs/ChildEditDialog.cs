...
namespace Project_kindergarten.Dialogs
{
    internal sealed class ChildEditDialog : Form
    {
        ...
        public ChildEditDialog(ChildData? existing = null)
        {
            Text = existing == null ? "Додати дитину" : "Редагувати дитину";
        ...
        private void BuildUi()
        {
            ...
            var lbl1 = new Label { Text = "ПІБ дитини", AutoSize = true, Location = new Point(xLbl, 20) };
            ...
            var lbl7 = new Label { Text = "Група", AutoSize = true, Location = new Point(xLbl, 390) };
        ...
        private bool TryBuildModelFromUi()
        {
            ...
            if (name.Length < 3)
            {
                MessageBox.Show("Введи коректне ПІБ дитини (мінімум 3 символи).", "Помилка", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return false;
            }

            if (_cbGroup.SelectedValue is not int groupId || groupId <= 0)
            {
                MessageBox.Show("Обери групу.", "Помилка", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return false;
            }
